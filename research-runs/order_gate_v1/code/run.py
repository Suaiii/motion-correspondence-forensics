...
def current_code_hashes():
    return {str(p): digest(p) for p in [ROOT/'run.py', ROOT/'model.py', ROOT/'video_io.py', ROOT/'selection.py', SAMPLER]}


def prepare(run):
    if run.exists():raise FileExistsError('Refuse overwrite; create a fresh run directory')
    cfg=read(ROOT/'protocol.json')
    prior=Path(cfg['input_run']); old_rows=read(prior/'selection.json')
    old_obs=[json.loads(s)for s in (prior/'observations.jsonl').read_text(encoding='utf-8').splitlines()]
    old_hashes={r['sha256']for r in old_obs}
    source=Path(cfg['source_manifest'])
    source_rows=[json.loads(s)for s in source.read_text(encoding='utf-8').splitlines()if s.strip()]
    rows=select(source_rows,old_rows,cfg['selection_seed'])
    run.mkdir(parents=True)
    shutil.copyfile(ROOT/'protocol.json',run/'protocol.json')
    dump(run/'selection_before_probe.json',rows)
    dump(run/'selection_lock.json',{'protocol_sha256':digest(run/'protocol.json'),'selection_sha256':digest(run/'selection_before_probe.json'),'source_manifest_sha256':digest(source),'prior_selection_sha256':digest(prior/'selection.json'),'prior_observations_sha256':digest(prior/'observations.jsonl')})
    for i,r in enumerate(rows):
        cap=cv2.VideoCapture(r['path'],cv2.CAP_FFMPEG,[cv2.CAP_PROP_N_THREADS,1])
        try:
            if not cap.isOpened():raise RuntimeError('Cannot open source '+r['sample_id'])
            fps=float(cap.get(cv2.CAP_PROP_FPS));n=int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
            w=int(cap.get(cv2.CAP_PROP_FRAME_WIDTH));h=int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
        finally:cap.release()
        if fps<=0 or n<=0 or w<=0 or h<=0:raise ValueError('Invalid metadata '+r['sample_id'])
        r.update(fps=fps,frame_count=n,duration_sec=n/fps,width=w,height=h,sha256=digest(r['path']))
        r['hash_matches_prior_pilot']=r['sha256'] in old_hashes
        if (i+1)%100==0:print('PREPARE',i+1,'/ 600',flush=True)
    counts=Counter(r['sha256']for r in rows)
    for r in rows:r['selected_hash_collision']=counts[r['sha256']]>1
    for src in (ROOT/'run.py',ROOT/'model.py',ROOT/'video_io.py',ROOT/'selection.py',SAMPLER):
    ...
    if env.exists():shutil.copyfile(env,run/'environment.json')
    dump(run/'lock.json',{'created_utc':datetime.now(timezone.utc).isoformat(),'code_hashes':current_code_hashes(),'protocol_sha256':digest(run/'protocol.json'),'input_manifest_sha256':digest(run/'input_manifest.json'),'torch':torch.__version__,'python':sys.version,'python_executable':sys.executable,'scope':'bounded local expanded development gate; no confirmatory test','main_thread':'01a055fc-51ee-7b00-824b-087fc85b70d5'})
    event(run,stage='prepare',samples=len(rows))
    print('PREPARED',len(rows),'previous-pilot hash matches',sum(r['hash_matches_prior_pilot']for r in rows),'selected duplicate hashes',sum(r['selected_hash_collision']for r in rows),flush=True)
...
def cache(run):
    ...
    buffers = {(c,m):[] for c in cfg['conditions'] for m in cfg['cache_representations']}
    ...
    for i, row in enumerate(rows):
        ...
        if digest(row['path']) != row['sha256']:
            ...
            break
        if row['hash_matches_prior_pilot'] or row['selected_hash_collision']:
            exclusions.append({'sample_id':row['sample_id'],'source':row['source'],'role':row['role'],'label_fake':row['label_fake'],'reason':'byte_duplicate_or_prior_pilot_hash'})
            continue
        ...
        try:
            ...
            for condition in cfg['conditions']:
                ...
                for model in cfg['cache_representations']:
    ...
    for role in ['fit','calibration','audit']:
        for label in [0,1]:
            assert sum(r['role']==role and r['label_fake']==label for r in kept) >= cfg['quality']['minimum_per_role_class']
...
def load_array(run, condition, kind):
    representation=read(run/'protocol.json')['representation_map'][kind]
    return np.load(run/'cache'/f'{condition}__{representation}.npy', mmap_mode='r', allow_pickle=False)
...
def evaluate(run):
    ...
    lines=['# 扩大开发样本与无序池化对照','', '本轮为本地开发 pilot，不是论文主结果。全部模型从随机初始化训练，未下载预训练权重。', '',
    ...
    lines+=['','同一clean开发校准集选择 checkpoint 与阈值，阈值固定应用于三种条件。audit从未用于训练早停或改分数方向。种子标准差不是数据集置信区间。',
