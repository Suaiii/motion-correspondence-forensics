...
> **当前版本：cvpr27-20260918-v1.4。** [持续科研规则](CONTINUOUS_RESEARCH_20260918.md)使算法探索独立于付费/真实输入阻塞；[新算法计划](ALGORITHM_SEARCH_20260918.md)检验H-J/H-T，C保留冻结参考。新49714服务器与既有公钥登录已核验；正式资源、数据及机制门槛仍独立验收。当前任务以project.json为准，下方旧“当前”字样均属历史。

> **2026-09-17 持续协作已获用户授权：** [执行补充](COLLABORATION_RUNBOOK_2026-09-17.md) 在 v1.1 科学方案之上增加协作调度，已纳入 DX01 准备诊断和 DX02 离线协议草案。当前任务数量与状态以 project.json 为准；11 个里程碑、预算与正式科学门槛不变。

> **2026-09-17 当前修订：cvpr27-20260917-v1.1。** [共享计划](WORK_PLAN.md) 增加静态重复帧/静态 patch 对照；RS04/RS05 仅软件验收通过，数据与机制门槛未通过。预算、日期和提醒不变。以下 v1 部署说明及更早内容保留为历史。

> **2026-09-17 当前唯一入口：[共享工作计划](WORK_PLAN.md)。** 计划版本 cvpr27-20260917-v1，角色入口见工作区根目录 AGENTS.md；25 项当前任务与 11 个里程碑见 task-hermes/project.json。累计预算 6000 元、初始累计 180 GPU 小时，先核减已用额度；仅里程碑提醒。下方所有旧“当前/最新”字样均属于带日期的历史记录，不覆盖新共享计划。旧任务表原样保存在 task-hermes/project_before_shared_plan_20260917.json。
