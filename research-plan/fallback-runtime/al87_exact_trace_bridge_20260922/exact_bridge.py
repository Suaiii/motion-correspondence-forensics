"""AL87 exact-byte precomputed four-call identity bridge; no model invocation."""
...
def _same(a, b):
    # Exact canonical float64 bytes; no numeric tolerance for identity fields.
    aa = np.ascontiguousarray(a, dtype=np.float64)
    bb = np.ascontiguousarray(b, dtype=np.float64)
    return aa.shape == bb.shape and aa.tobytes(order="C") == bb.tobytes(order="C")
...
def bridge(trace):
    ...
    k_from_d = da - db
    if not _close(k_direct, k_from_d):
