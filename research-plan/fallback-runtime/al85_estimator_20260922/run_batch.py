...
def main():
    ...
    for name in cfg["reject_cases"]:
        try:
            core.macro(mutate(cfg,name)["records"],mutate(cfg,name)["scores"],mutate(cfg,name)["weights"],methods,tags,gens,ancestors)
