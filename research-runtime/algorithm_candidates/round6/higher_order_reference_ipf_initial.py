...
def binary_maxent(p):
    """IPF from uniform; constraints are declared empirical payload pair tables."""
    target=pair_marginals(p);out=np.ones((2,2,2))/8
    for iteration in range(10000):
        for pair,q in enumerate(target):
            current=pair_marginals(out)[pair]
            if np.any((current==0)&(q>0)):raise ValueError('Infeasible support')
            factor=np.divide(q,current,out=np.zeros_like(q),where=current>0)
            out*=factor[:,:,None] if pair==0 else factor[None,:,:] if pair==1 else factor[:,None,:]
        if max(abs(a-b).max() for a,b in zip(pair_marginals(out),target))<1e-12:
            return out,iteration+1
    raise RuntimeError('Reference IPF did not converge; no silent substitute')
...
def infer(keys,z,tau=.1,override_w=None):
    ...
    return dict(tensor=t,delta=delta,reference_tensor=tref,scalar_signed=float(delta[0,0,0]),
        ...
        reference_ipf_iterations=iters,full_index_reference_delta=full_reference_delta,
...
def compact(r):
    keys=('score','scalar_signed','ordinary_aligned_score','unaligned_score','full_index_reference_delta',
          ...
          'static_payload_third','payload_pair_tables','reference_ipf_iterations')
