"""Execute exactly the AL27 frozen batch; refuse output overwrite or source drift."""
import os
...
def read(name):
    return json.loads((OUT/('AL27_'+name+'.json')).read_text(encoding='utf-8'))
...
def main():
    if (OUT/'AL27_evidence.json').exists() or (OUT/'AL27_started.json').exists():
    ...
    for relative in [*frozen['inputs'], 'research-runs/algorithm_search_20260918/AL27_freeze.json']:
    ...
    save('started', dict(commit=commit, started_at=datetime.now(timezone.utc).isoformat(),
                         freeze_sha256=sha(OUT/'AL27_freeze.json'), batch='single_fixed_batch'))
    wall_start, cpu_start = time.perf_counter(), time.process_time()
    evidence = dict(commit=commit, freeze_sha256=sha(OUT/'AL27_freeze.json'), checks=[],
    ...
    try:
        ...
        def run_head(panel, head):
            ...
            return solve(differences,reg,config['solver'])
        for panel in inputs['panels']:
            entry = dict(name=panel['name'], pairs=len(panel['plus']), heads={})
            ...
            if panel['name'] in ['aligned','full_rank']:
                ...
                if panel['name']=='aligned':
                    ...
                    check('full_rank/fixed_feasible_below_log2',fixed['objective']<math.log(2))
                    check('full_rank/free_positive_and_better',bool(np.all(beta>0)) and
        ...
        evidence['scope'] = 'multidimensional_objective_reference_software'
