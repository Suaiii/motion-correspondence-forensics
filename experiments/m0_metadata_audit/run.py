...
def review_check(run, rows, candidates):
    if not candidates:
        return
    path = run / "candidate_decisions.json"
    if not path.is_file():
    ...
    for candidate in candidates:
        ...
        matches = [d for d in decisions if set(d["sample_ids"]) == pair]
        if len(matches) != 1 or matches[0]["verdict"] != "not_duplicate":
            raise ValueError("Missing or negative duplicate review verdict")
        decision = matches[0]
        if not decision.get("reason") or not decision.get("reviewer") or not decision.get("evidence"):
            raise ValueError("Incomplete duplicate review")
        for sid in pair:
            if decision["video_sha256"].get(sid) != lookup[sid]["sha256"]:
                raise ValueError("Duplicate review video hash mismatch")
        for evidence in decision["evidence"]:
            target = Path(evidence["path"])
            if not target.is_absolute() or target.drive.upper() != "E:" or sha(target) != evidence["sha256"]:
                raise ValueError("Duplicate review evidence missing or changed")
...
def evaluate(run):
    ...
    review_check(run, rows, receipt["near_duplicate_candidates"])
    ...
    lines.extend(["", "120 条视频：60 real、30 OpenSora、30 t2vz；fit/calibration/audit=72/24/24。real 每个原视频前缀最多一条；fake 仅以路径和字节哈希初步去重，prompt/父生成视频映射仍缺失。", "",
                  ...
                  f"校验：所选文件 SHA256 无重复；两时点 average hash 标记 {len(receipt['near_duplicate_candidates'])} 对跨角色候选，已用带图像证据和哈希的决定逐项复核。S019 抽检帧近乎全黑，S038 有动画片头；原始资产诊断保持样本，不静默替换。该检查不等于完整内容去重或真实性认证。", "",
